    ...
    <tbody>
      <tr>
					...
					while($row = mysqli_fetch_array($result))
						{
							...
							echo '<br>';
							echo "<form action=doctordelete.php?edit=$row[Id] method=post>
        ...
        <td><div id="content">
			<?php
			$host=mysqli_connect("localhost","root","","spotter");
			$result = mysqli_query($host,"SELECT * FROM camp");
					while($row = mysqli_fetch_array($result))
						{
							echo '<li>'.$row['message'].'</li>';
							echo '<br>';
							echo "<form action=campdelete.php?edit=$row[Id] method=post>
							<input type=submit value=Delete>
							</form>
							";
							echo '<hr>';
